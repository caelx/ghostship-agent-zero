...
- Agent Zero plugins install only through upstream `helpers.plugins.find_plugin_dir(name)`, backed by `files.get_abs_path(files.USER_DIR, files.PLUGINS_DIR)`; do not assume a fixed physical plugin root because it depends on upstream Agent Zero runtime context.
